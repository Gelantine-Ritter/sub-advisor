           ...
           v-if="dateObj">
            <v-col cols="12" xs="12" sm="8" md="8" lg="7" xl="7">
              <v-card class="pa-4" tile flat>
                <v-row class="ma-1">
                <h6 class="mr-1 font-weight-bold text--disabled">{{ dateObj.date }} //</h6>
                <h6 class="mr-1 font-weight-bold text--disabled"> {{ dateObj.eventStart }} -
                  {{ dateObj.eventEnd }} // </h6>
                <h6 class="font-weight-bold text--disabled"><ParticipantsIcon  :number="highest.amountOfGuests"/>
                </h6>
                </v-row>
                <v-row class="ma-1">
            ...
            <v-col>
              ...
              <v-card  class="pa-2 rounded-xl myPictureCard" flat tile>
                <v-responsive>
                  ...
                  <template v-else>
                    <v-img fill class="myPicture" :src="picDataUrl(highest.pic)" alt="" />
                  </template>
                </v-responsive>
              </v-card>
              <!-- EVENT PICTURE END -->
...
export default {
  ...
  mounted(){
    ...
    requestProvider.getEventsForDateVenueTag(null,date1,null).then((response)=> {
      ...
      //  get most joined Event
      var highestLocal = myData[0]
      for ( var i = 0; i< myData.length; i++){        
      if(myData[i].amountOfGuests > highestLocal.amountOfGuests){
          highestLocal = myData[i]     
          
      }}
      this.highest = highestLocal

     requestProvider.getVenue(this.highest.venueId).then((responseVenue) => {
     ...
     this.alreadyJoined()
      ...
      this.dateObj = {
          eventStart: DateConverter.getTime(highestLocal.eventStart),
          eventEnd: DateConverter.getTime(highestLocal.eventEnd),
          date: DateConverter.getDate(highestLocal.date),
          artists: this.convertArtists(highestLocal.artists),
      }
    }) 
  },
  methods: {
